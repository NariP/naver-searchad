...
## ADR-013 — `.env` 폴백 추가 (보안저장소·환경변수 다음 3순위)

**Date**: 2026-06-19
**Status**: Accepted

### Context
리눅스/CI엔 키체인·자격증명관리자가 없어 매번 export해야 함. 키 공급에 편의 폴백이 필요.

### Decision
`nsa.py`에 `.env` 폴백 추가. 우선순위: ①환경변수 ②OS 보안저장소(래퍼가 ①로 변환) ③`.env`(평문). 표준 라이브러리로 파싱(의존성 0), 따옴표·공백·주석 처리. 탐색: `NSA_DOTENV` > cwd > 레포 루트. `.gitignore`에 `.env` 이미 포함.

### Rationale
보안저장소가 없는 환경(리눅스/CI/윈도우 비번거로움)의 편의. 평문이라 보안저장소가 가능하면 그쪽 권장(문서 명시). 환경변수가 있으면 `.env`는 덮어쓰지 않음(명시적 우선). doctor가 `.env` 출처를 구분 표시.
