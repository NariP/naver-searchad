...
**단일 스킬** `naver-searchad` — 조회·키 셋업·진단을 모두 포함하고, 실행체·문서를 스킬 폴더 안에 자체 보유한다.

```
skills/naver-searchad/
├── SKILL.md
├── scripts/      # nsa.py(본체, 의존성 0) · nsa(mac) · nsa.ps1(win)
└── references/   # endpoints·fields + 정책 문서
```

이 구조라 `skills add` 또는 폴더 링크 한 번으로 완결 설치된다(scripts·references가 같이 따라옴).
...
```
> 레포는 `~/.naver-searchad`(env `NSA_HOME`)에 받아두고, 스킬을 심볼릭링크한다.
...
설치 후 아래 순서대로 진행한다. 스크립트는 스킬 폴더 안 `skills/naver-searchad/scripts/` 에 있다(아래 예시는 그 폴더 기준 `S=skills/naver-searchad/scripts`).

## 사용

### 1) 환경 진단 (어느 OS든)
```bash
python3 skills/naver-searchad/scripts/nsa.py doctor
```
...
| OS | 키 보관 | 셋업 |
|---|---|---|
| **macOS** | 키체인 | `skills/naver-searchad/scripts/nsa init` |
| **Windows** | 자격증명관리자 | `.\skills\naver-searchad\scripts\nsa.ps1 init` |
...
### 3) 조회
```bash
S=skills/naver-searchad/scripts   # macOS/Linux 편의 변수

# macOS
$S/nsa campaigns
$S/nsa stats --ids <campaignId> --since 2026-06-01 --until 2026-06-17

# Windows
.\skills\naver-searchad\scripts\nsa.ps1 stats --ids <campaignId> --since 2026-06-01 --until 2026-06-17

# 환경변수/.env 직접 (어느 OS든)
python3 $S/nsa.py campaigns
```
...
엔드포인트별 상세·필드는 `skills/naver-searchad/references/endpoints.md`. 네이버가 엔드포인트·필드를 바꾸면 스킬 마이너 버전을 올리고 이 표에 추가한다.
