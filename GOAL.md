...
| 단계 | 핵심 변화 |
|---|---|
| ✅ **Day 1** | `nsa.py` 구현 (전 명령) + SKILL.md + mock 검증. DoD 6/6 통과. |
| ✅ **Day 2** | 키 투입(Keychain, 계정 `howbuild`) → 라이브 검증 완료. `campaigns`/`stats` 실데이터 확인, CTR/CPC 일치. |
| Day 3+ | (선택) CSV 출력 옵션, report 폴링 견고화, 페이지네이션, 730일 초과 시 기간 자동 클램프 안내 |

### Day 2 라이브 검증 결과 (2026-06-18)

- 계정: `customerId 472828`(howbuild), 캠페인 72개
- `campaigns` 정상, `stats` 8개 캠페인 노출/클릭/광고비/CTR/CPC 실데이터 확인
- 한글 라벨 정상, API ctr/cpc ↔ 직접계산 일치 (예: 688/48532=1.42%, 472719/688=687)
- 에러 핸들링 확인: 730일 초과 → API 11004 메시지 그대로 노출, PAUSED 캠페인 → 빈 결과(정상)
